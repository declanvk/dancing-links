//! Dancing links `Grid` implementation for use in the `Solver`. The benefit of
//! this grid is that it is sparse and easily manipulated to cover and uncover
//! rows and columns.
...
use core::{
    iter::{self, once, Chain, Map, Once},
    ptr,
};
use std::collections::VecDeque;

use self::base_node::BaseNodeIterator;
...
impl Grid for SparseGrid {
    type Column = *mut Column;
    type Row = *const Node;
    type UncoveredColumnsInRowIter = Chain<
        Map<
            Map<
                Map<
                    BaseNodeIterator<fn(&BaseNode) -> *mut BaseNode>,
                    fn(*mut BaseNode) -> *const BaseNode,
                >,
                fn(*const BaseNode) -> *const Node,
            >,
            fn(*const Node) -> *mut Column,
        >,
        Once<*mut Column>,
    >;
    type UncoveredColumnsIter =
        Map<BaseNodeIterator<fn(&BaseNode) -> *mut BaseNode>, fn(*mut BaseNode) -> *mut Column>;
    type UncoveredRowsIter = Map<
        Map<BaseNodeIterator<fn(&BaseNode) -> *mut BaseNode>, fn(*mut BaseNode) -> *const BaseNode>,
        fn(*const BaseNode) -> *const Node,
    >;

    fn new(num_columns: usize, filled_coordinates: impl IntoIterator<Item = (usize, usize)>) -> Self
    where
        Self: Sized,
    {
        SparseGrid::new(num_columns, filled_coordinates)
    }
    ...
    fn uncovered_columns_in_row(&self, row: Self::Row) -> Self::UncoveredColumnsInRowIter {
        Node::neighbors(row)
            .map(Node::column_ptr as fn(*const Node) -> *mut Column)
            .chain(iter::once(Node::column_ptr(row)))
    }
}
...
impl SparseGrid {
    ...
    pub fn uncovered_columns(
        &self,
    ) -> Map<
        Map<BaseNodeIterator<fn(&BaseNode) -> *mut BaseNode>, fn(*mut BaseNode) -> *const BaseNode>,
        fn(*const BaseNode) -> *const Column,
    > {
